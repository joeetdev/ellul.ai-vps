...
function initWorkspaceFiles(workspace: string, projectName: string): void {
  try {
    ...
    //
    // NOTE: AGENTS.md is NOT written here — it's generated by ellulai-ctx
    // with full platform context for CLI tools (OpenCode, Claude, Gemini).
    // OpenClaw's workspace rules are already in its system prompt.
...
export async function ensureProjectAgent(projectName: string): Promise<boolean> {
  ...
  if (!existsSync(claudeMdPath)) {
    refreshProjectContext(workspace);
  }
