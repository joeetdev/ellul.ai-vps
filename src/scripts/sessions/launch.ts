...
case "$SESSION" in
  ...
  git)
    ...
    ;;
  save)
    exec tmux new-session -A -s save "phonestack-ai-flow save; exec bash"
    ;;
  ship)
    exec tmux new-session -A -s ship "phonestack-ai-flow ship; exec bash"
    ;;
  branch)
    exec tmux new-session -A -s branch "cd /home/dev/projects/welcome && phonestack-git-flow branch; exec bash"
    ;;
  undo)
    exec tmux new-session -A -s undo "phonestack-undo; exec bash"
    ;;
  logs)
    exec tmux new-session -A -s logs "echo -e '\\033[32mLIVE LOGS - Press Ctrl+C to exit\\033[0m' && pm2 logs --lines 50; exec bash"
    ;;
  clean)
    exec tmux new-session -A -s clean "sudo phonestack-clean; exec bash"
    ;;
  *)
